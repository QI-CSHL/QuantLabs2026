...
1. Smoothing
2. {term}`Thresholding<Threshold>`
3. Filling holes in threshold {term}`masks<Mask>`
...
- If you click the `Step` button  <img src="images/processing_detection/Step.png" height="30px" />, you will execute the *just* next (active) module.
- If you click the `Run` button  <img src="images/processing_detection/Run.png" height="30px" />, all the remaining (active) modules will be executed, until the next active module marked with `Pause` <img src="images/processing_detection/Pause.png" height="30px" />.
```

### Load files
- Open a new CellProfiler window, or open a clean starting version by going to File -> New Project  
- Load the `Watershed_CellProfiler` folder into top-right space in the **Images** module.
  - This will load 3 sets of images, each with a DAPI image and and actin image
- Load the `basic_segmentation.cppipe` pipeline file into the pipeline panel (left panel in the CellProfiler window)
...
### {term}`Threshold` and fill holes in the threshold masks

- Run the `Threshold` module on the smoothed image
- Run the `RemoveHoles` module on the thresholded image to remove any holes in the {term}`masks<Mask>`.
...
- Run the `Watershed` module to turn your {term}`masks<Mask>` into actual CellProfiler objects.
...
```{caution}
How do you know when your {term}`segmentation` is "good enough"? It's a _SUPER_ common but a very complicated question! You can check out [this blog post](https://carpenter-singh-lab.broadinstitute.org/blog/when-to-say-good-enough) for an attempt at helping come up with a set of rules for making that decision.
```

## **{term}`Segmentation` - How we'd do it in the "real world"**
...
- What settings correspond to our {term}`segmentation` steps, as in `IdentifyPrimaryObjects`?
...
- Download the `DL4MIA/hard/train` dataset from [this link](https://drive.google.com/drive/folders/1vkVes-FApU1WdeXKWLJlC-8P_rAyy0CD).
...
- Use the <img src="images/processing_detection/AddModule.png" height="25px" /> to open the `Add modules` window.
...
We've provided you some images {cite}`Gustafsdottir2013-ng` from the Broad Bioimage Benchmark Collection {cite}`Ljosa2012-bf` that have been treated with a set of organelle dyes collectively called Cell Painting. This assay will be discussed later in the course during Beth's seminar (and these same images will show up tomorrow).  The
...
```{figure} ./images/advanced_seg_bonus/Fig1.png
:width: 700
:align: center
...
```{figure} ./images/advanced_seg_bonus/Fig2.png
:width: 600
:align: center
...
* Download the images, pipelines, and metadata files [from here](https://drive.google.com/drive/folders/1DrDhyEmhN7BqNowx4-mSW508VSzg-kvs) 
...
- The fourth metadata extraction step requires you to tell CellProfiler the
  ...
  - Select the  <img src="./images/advanced_seg_bonus/OutputSettings.png" width="120"/> button in the bottom left corner of the screen.
...
```{figure} ./images/advanced_seg_bonus/Fig3.png
:width: 700
:align: center
...
```{figure} ./images/advanced_seg_bonus/Fig4.png
:width: 600
:align: center
...
```{figure} ./images/advanced_seg_bonus/Fig5.png
:width: 600
:align: center
...
```{figure} ./images/advanced_seg_bonus/Fig6.png
:width: 600
:align: center
...
```{figure} ./images/advanced_seg_bonus/Fig7.png
:width: 600
:align: center
...
```{figure} ./images/advanced_seg_bonus/Fig8.png
:width: 700
:align: center
...
```{figure} ./images/advanced_seg_bonus/Fig9.png
:width: 400
:align: center
